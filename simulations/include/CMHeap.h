...
//    unordered_map<string, uint32_t> ht;
    ...
    void heap_adjust_down(int i) {
        while (i < heap_element_num / 2) {
            ...
            int r_child = 2 * i + 2;
            int small_one = i;
            if (l_child < heap_element_num && heap[l_child].second < heap[small_one].second) {
                small_one = l_child;
            }
            if (r_child < heap_element_num && heap[r_child].second < heap[small_one].second) {
                small_one = r_child;
            }
            if (small_one != i) {
                swap(heap[i], heap[small_one]);
//                swap(ht[string((const char *)heap[i].first, key_len)], ht[string((const char *)heap[larger_one].first, key_len)]);
                swap(ht[heap[i].first], ht[heap[small_one].first]);
                heap_adjust_down(small_one);
            } else {
                break;
            }
        }
