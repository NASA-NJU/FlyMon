# ifndef __ES_LIGHT_PART_H_
# define __ES_LIGHT_PART_H_
...
public:
	...
	void insert(const uint8_t *key, int key_len, int f = 1);
	void swap_insert(const uint8_t *key, int key_len, int f);
	int query(const uint8_t *key, int key_len) ;

    void compress(int ratio, uint8_t *dst);
    int query_compressed_part(const uint8_t *key, int key_len, uint8_t *compress_part, int compress_counter_num);
...
template<int init_mem_in_bytes>
void LightPart<init_mem_in_bytes>::insert(const uint8_t *key, int key_len, int f)
{
    uint32_t hash_val = (uint32_t)bobhash->run((const char*)key, key_len);
...
template<int init_mem_in_bytes>
void LightPart<init_mem_in_bytes>::swap_insert(const uint8_t *key, int key_len, int f)
{
    uint32_t hash_val = (uint32_t)bobhash->run((const char*)key, key_len);
    uint32_t pos = hash_val % (uint32_t)counter_num;
...
template<int init_mem_in_bytes>
int LightPart<init_mem_in_bytes>::query(const uint8_t *key, int key_len) 
{
    uint32_t hash_val = (uint32_t)bobhash->run((const char*)key, key_len);
    uint32_t pos = hash_val % (uint32_t)counter_num;
...
template<int init_mem_in_bytes>
int LightPart<init_mem_in_bytes>::query_compressed_part(const uint8_t *key, int key_len, uint8_t *compress_part, int compress_counter_num) 
{
    uint32_t hash_val = (uint32_t)bobhash->run((const char *)key, key_len);
